...
def server(input, output, session):
    ...
    @render_widget
    def stages_won():
        stage_wins = stages_data()[stages_data()['rank'] == 1].groupby('rider').size().reset_index(name='count')
    ...
    @render_widget
    def stages_ridden():
        stages_completed = stages_data().groupby('rider').size().reset_index(name='count')
        stages_completed = stages_completed.sort_values('count', ascending=False).head(5)
    ...
    def stage_time_distribution_plot():
        ...
        for year in sorted(filtered_data['year'].unique()):
            ...
            year_clean = year_data[(year_data['elapsed'] >= (Q1 - 1.5 * IQR)) & 
        ...
        for year in years:
            ...
            x_vals = np.linspace(min(clean_data['elapsed']), max(clean_data['elapsed']), 1000)
            y_vals = kde(x_vals)
    ...
    def age_distribution_plot():
        unique_riders = stages_data().drop_duplicates(subset=['rider', 'year'])[['rider', 'year', 'age']]
        ...
        for year in years:
            ...
            x_vals = np.linspace(min(unique_riders['age']), max(unique_riders['age']), 1000)
            y_vals = kde(x_vals)
    ...
    def attrition_plot():
        ...
        stage_attrition = stages_data().groupby(['year', 'stage_results_id']).agg(
            num_finishers=('rider', 'nunique')
        ).reset_index()

        # Calculate the maximum number of finishers for each year (typically the first stage)
        max_finishers = stage_attrition.groupby('year')['num_finishers'].transform('max')
        stage_attrition['pct_finishers'] = stage_attrition['num_finishers'] / max_finishers
        ...
        all_stages = sorted(stage_attrition['stage_results_id'].unique(), 
        ...
        for year in years:
            ...
            stage_info = [(stage, *extract_stage_components(stage)) 
            ...
            ordered_data = year_data.set_index('stage_results_id').loc[sorted_stage_ids].reset_index()
